...
export function createHttpViewportStationDiscovery<TStation extends IChargingBranch = IChargingBranch>({
  endpoint,
  headers,
}: HttpViewportStationDiscoveryOptions): ViewportStationDiscovery<TStation> {
  return {
    findInBounds: (query) => {
...
export function quantizeBounds(bounds: IMapBounds, precision = 0.05): IMapBounds {
  ...
  return { south: floor(bounds.south), west: floor(bounds.west), north: ceil(bounds.north), east: ceil(bounds.east) };
}
