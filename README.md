...
- **PWA**: manifest, Serwist service worker (network-first navigation /
  cache-first static assets / offline document fallback), install prompt, push
  subscribe/unsubscribe/topics (mocked via local Route Handlers when enabled),
  automatic service-worker update/activation, foreground refetch on
  `visibilitychange`/`pageshow`, on-screen-keyboard inset tracking, and
  network-status banner. Icons in `public/icons` are placeholders.
...
The service worker is generated by **Serwist** at `/serwist/sw.js` from
`src/app/sw.ts`. The application does not use the old custom `public/sw.js`,
`public/sw.template.js`, or `generate-sw.mjs` architecture.

The current update flow is automatic:

1. `SerwistProvider` registers `/serwist/sw.js`.
2. `ServiceWorkerUpdate` checks for a newer worker when the app starts and when
   the app returns to the foreground or comes back online.
3. A new worker is allowed to finish installing and enters the waiting state.
4. `ServiceWorkerUpdate` sends the skip-waiting message automatically when no
   React Query mutation is running.
5. If a mutation is active, activation is deferred until the mutation cache
   reports that mutations have finished.
6. Once the new worker controls the page, the app reloads once and runs the new
   version.

There is **no user-facing update button and no DevTools `Skip waiting` step**
for normal releases. The service worker keeps `skipWaiting: false` so the app
can coordinate activation safely instead of forcing activation during an
active mutation.
...
- reopen or foreground the existing PWA;
- version B's worker must be discovered and activated automatically;
- the page must reload into version B;
- no manual DevTools action must be required.

This must still be verified in a real browser and standalone PWA before calling
the update path production-ready.

### iOS / iPadOS

- The app uses the browser's native geolocation permission flow.
- The app uses the browser's native notification permission flow.
- Web Push is exposed only when the browser supports it; on iOS/iPadOS the app
  requires standalone Home Screen mode.
- iOS/iPadOS startup splash entries must reference files that actually exist in
  `public/splash`.
- Neshan handoff still requires real-device verification on iOS because the
  installed-app URL scheme is provider/device dependent.

### Push notifications

`usePushSubscription` handles the browser subscription lifecycle and sends the
subscription to the backend through the existing mutation hooks. The public
VAPID key is read from `NEXT_PUBLIC_VAPID_PUBLIC_KEY`; the hook now fails
explicitly when that variable is missing instead of passing `undefined` into
`pushManager.subscribe()`.

Development can use `NEXT_PUBLIC_PUSH_MOCK=true` and the local push route
handlers. Production must provide the real VAPID public key and replace the
mock push API with the backend push-subscription/notification implementation.
...
- [ ] Perform a controlled service-worker A -> B deployment test without
      clearing browser storage or manually skipping the waiting worker.
...
endpoints, mock toggles, `NEXT_PUBLIC_VAPID_PUBLIC_KEY`, and API base URL.
