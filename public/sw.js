...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname.startsWith('/_next/static/') || /\.(?:js|css|png|jpg|jpeg|svg|webp|woff2?|ico)$/.test(url.pathname)) {
...
function resolveNotificationContent(payload) {
  switch (payload.type) {
    ...
    case 'charging_session':
      ...
      return { title: payload.title, body: payload.body, url: `/wallet/sessions/${payload.sessionId}` };
    case 'wallet':
